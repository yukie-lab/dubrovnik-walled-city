...
function cloudAtlas(rng, { size = 1024 }) {
  ...
  for (let cy = 0; cy < 2; cy++) for (let cx = 0; cx < 2; cx++) {
    ...
    // **雲は自分のセルの中に収まらなければならない。**
    // 塊の中心は y ≤ 0.92、半径は 0.26 まであり、縦は dd の 0.85 で 1/0.85 に
    // 伸びるので、支持域は v = 1.23 まで届く。セルの外は評価されないので、
    // そこは **真横の直線で切られる**(実測 tools/_cloudatlas.mjs: 4 セル中
    // 2 セルの上辺に α 0.34 / 0.57 が残っていた)。雲板は 4 種類しかないので、
    // 切られた 1 枚が空のあちこちに出る(ユーザー報告「雲の上が直線」)。
    // 下は平底のぼかしが v≈0.19 で 0 にするので切れない。上と左右を抑える。
    const CM = 0.02;                                   // セル縁の余白
    for (const b of blobs) {
      b.r = Math.min(b.r, (1 - 2 * CM) / 2 * 0.85);    // 大きすぎる塊はセルに入らない
      const rv = b.r / 0.85;                           // 縦の支持半径
      b.x = clamp(b.x, CM + b.r, 1 - CM - b.r);
      b.y = Math.min(b.y, 1 - CM - rv);
    }

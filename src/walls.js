...
export function makeWalls(plan, tex, stepPool, outsideHeight) {
  ...
  for (const g of railGaps) {
    ...
    // 踊り場は **上面と前板しか無かった**。歩廊の描かれたデッキより外へ
    // 張り出しているので(実測 #3 は中央から内端まで真下 10.5m に石が無い)、
    // 横から見ると宙に浮いた 1 枚の板になっていた
    // (ユーザー報告「階段の一番上が板になっている」)。
    // 石は厚みを持つ。持ち送りの塊として、前・両側・底を閉じる。
    const D = 2.2;                       // 持ち送りの丈
    const c0 = [e[0] + px2 * wP, e[1] + pz2 * wP];
    const c1 = [e[0] - px2 * wN, e[1] - pz2 * wN];
    const c2 = [inEdge[0] - px2 * wN, inEdge[1] - pz2 * wN];
    const c3 = [inEdge[0] + px2 * wP, inEdge[1] + pz2 * wP];
    // 前(階段側)
    quad([c0[0], y - D, c0[1]], [c1[0], y - D, c1[1]],
         [c1[0], y, c1[1]], [c0[0], y, c0[1]], 0.85, 0.95);
    // 側面 二枚(巻きは互いに逆 — 揃えると片方が裏面になって消える)
    quad([c1[0], y - D, c1[1]], [c2[0], y - D, c2[1]],
         [c2[0], y, c2[1]], [c1[0], y, c1[1]], 0.85, 0.95);
    quad([c3[0], y - D, c3[1]], [c0[0], y - D, c0[1]],
         [c0[0], y, c0[1]], [c3[0], y, c3[1]], 0.85, 0.95);
    // 底(下から見上げても抜けない)
    quad([c0[0], y - D, c0[1]], [c3[0], y - D, c3[1]],
         [c2[0], y - D, c2[1]], [c1[0], y - D, c1[1]], 0.80, 0.90);
  }

...
void main() {
  ...
  // 帯がまっすぐ横に走ると、雲の頭に重なった所で **雲の上が定規で切ったように**
  // 見える(ユーザー報告)。実在の霞の層は波打つ。低周波の noise で座標そのものを
  // 歪ませれば、異方性(13.0 = 水平に広く垂直に薄い)も層の縁も残したまま、
  // **直線だけが消える**。
  // 歪みに階層は要らない(滑らかにずらすだけ)。fbm3 は noise3 を 3 回
  // 呼ぶので、空の画素すべてで fps を 8% 落とした。1 回で足りる。
  float warp = noise3(d * vec3(1.3, 2.1, 1.3) + 31.7) - 0.5;
  vec3 brushP = d * vec3(3.4, 13.0, 3.4)
    + vec3(uHour * 0.55, uTime * 0.012 + warp * 2.6, uHour * 0.21);
